...
LENS 是由 Codex Skills 与可执行脚本组成的科研文献工作流。你可以直接把 PDF 交给 Agent，让它生成中文结构化 Markdown 文件、提取 Figure/Table、建立文献索引、跟踪新论文，并把 Markdown 文件转换成以论文图片为主的 PPT。
...
| 想做什么 | 可以直接这样说 |
| --- | --- |
| 阅读一篇论文 | `使用 LENS 读取这个 PDF，生成结构化 Markdown 文件。` |
...
把 PDF 作为附件发送，随后用自然语言提出任务。Agent 会读取本地 LENS 规则，执行对应脚本，并返回可打开的结构化 Markdown 文件路径和提取状态。

![在 Agent 中使用 LENS](assets/agent-usage-example.png)

### 结构化 Markdown 产物

Article Markdown 文件通常包含 YAML 元数据、Info、Summary、Intro、Works、Results、Question–Method Map、Discussion 和 Key references；Review 则以 Figure/Table 为锚点组织 Evidence。提取的图片保存在 Markdown 文件对应的 assets 目录。

![LENS 结构化 Markdown 文件示例](assets/paper-note-example.png)

### Figure 与证据解读

LENS 会将可靠定位的 Figure/Table 裁剪到对应 assets 目录，并把图片、原始 legend 和中文证据解读组织在同一个 Markdown 章节中。

![Figure 提取与证据解读示例](assets/figure-extraction-example.png)

结构化 Markdown 不止记录实验结果，也会整理 Prior Knowledge、Innovation、Limitations、Evidence Chain 和研究启示，方便后续批判性阅读与跨论文综合。

![Discussion、局限性与研究启示示例](assets/structured-markdown-discussion.png)

### 结构化文献 Wiki

LENS 可以根据结构化 Markdown 文件重建 Reading/Library Wiki，生成按主题和作者组织的导航页面、结构化索引数据及本地文献关系图，便于从单篇阅读扩展到研究领域知识网络。

![LENS 结构化文献 Wiki 与主题关系图](assets/structured-literature-wiki.png)

### 追踪每周新文献

配置研究方向和来源后，可以直接让 Agent 运行 literature follow-up。LENS 会抓取新论文、按 `USER_RESEARCH` 匹配，并给出 `read`/`watch` 建议及中文摘要。

![LENS 每周文献追踪示例](assets/literature-followup-example.png)

### 从 Markdown 文件生成 PPT

把结构化 Markdown 文件路径交给 Agent，并提出“把这篇文章的笔记做成 PPT”。LENS 会生成以 Figure/Table 为主的 PPTX，并检查标题、引用、图片布局和页面完整性。

![从 Markdown 文件生成 PPT](assets/ppt-generation-example.png)

生成后的 PPT 会按 Info、Intro、Summary 和 Results 组织内容，并包含章节过渡页、研究流程页及 Figure/Table 内容页。

<p align="center">
  <img src="assets/ppt-output-overview-1.png" alt="LENS PPT 前半部分页面预览" width="49%">
  <img src="assets/ppt-output-overview-2.png" alt="LENS PPT 后半部分页面预览" width="49%">
</p>
...
完整阅读 Article 或 Review PDF，并生成结构化中文 Markdown 文件。它还负责：
